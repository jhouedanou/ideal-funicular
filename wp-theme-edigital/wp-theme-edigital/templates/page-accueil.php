			...
			<div class="swiper-wrapper">
				...
				if ( $slides_query->have_posts() ) :
					while ( $slides_query->have_posts() ) : $slides_query->the_post();
						...
						$image_raw  = get_post_meta( get_the_ID(), 'slide_image', true );
						$video_raw  = get_post_meta( get_the_ID(), 'slide_video', true );
						...
						if ( function_exists( 'get_field' ) ) {
							$image_raw = get_field( 'slide_image' ) ?: $image_raw;
							$video_raw = get_field( 'slide_video' ) ?: $video_raw;
						}

						// Normalise $image en array {url, alt}. Accepte : array ACF, ID
						// d'attachment (string ou int), ou URL directe.
						$image = array( 'url' => '', 'alt' => '' );
						if ( is_array( $image_raw ) ) {
							$image['url'] = isset( $image_raw['url'] ) ? $image_raw['url'] : '';
							$image['alt'] = isset( $image_raw['alt'] ) ? $image_raw['alt'] : '';
						} elseif ( is_numeric( $image_raw ) && (int) $image_raw > 0 ) {
							$image['url'] = wp_get_attachment_url( (int) $image_raw );
							$image['alt'] = get_post_meta( (int) $image_raw, '_wp_attachment_image_alt', true );
						} elseif ( is_string( $image_raw ) && '' !== $image_raw ) {
							$image['url'] = $image_raw;
						}

						// Normalise $video. La maquette pointe sur un fichier statique du
						// thème ; on accepte aussi un ID d'attachment ou un array ACF.
						$video = array( 'url' => '' );
						if ( is_array( $video_raw ) ) {
							$video['url'] = isset( $video_raw['url'] ) ? $video_raw['url'] : '';
						} elseif ( is_numeric( $video_raw ) && (int) $video_raw > 0 ) {
							$video['url'] = wp_get_attachment_url( (int) $video_raw );
						} elseif ( is_string( $video_raw ) && '' !== $video_raw && '0' !== $video_raw ) {
							$video['url'] = ( strpos( $video_raw, 'http' ) === 0 || strpos( $video_raw, '/' ) === 0 )
								? $video_raw
								: get_template_directory_uri() . '/' . ltrim( $video_raw, '/' );
						}
